...
class Priority_ALGO():
    ...
    def calculate(self, write):
        ...
        if write:
            ...
            table = Table(title="[#D0FB59]Priority RESULT[/]")

            # adding the columns
            table.add_column("[cyan]Total Process Given[/]", style="cyan", no_wrap=True)
            table.add_column("[magenta]Total wait time[/]", style="magenta")
